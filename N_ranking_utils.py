...
class MLModel():
    ...
    def run(self,X_train, y_train,test_names,  X_tests, y_tests,result_base_file_name, feature_name, test_precisions, budget, save_feature_importance=False):
        ...
        result_base_file_name+='/'+feature_name
...
class Benchmark:
    ...
    def initialize_ela(self, scale_y=False):
        if scale_y:
            ela=pd.read_csv(f'data/{self.name}_{self.dimension}d_scaled.csv',index_col=[0])
        else:
            ela=pd.read_csv(f'data/{self.name}_{self.dimension}d.csv',index_col=[0])
        if 'f' in ela.columns:
            ela=ela.set_index('f')
    ...
    def initialize_precision(self, algorithm_portfolio,do_discrete_ranking):
        try:
            if do_discrete_ranking:
                self.precision=pd.read_csv(f'data/{self.version}_{self.dimension}d_{algorithm_portfolio}_ranks.csv',index_col=self.id_columns)
            else:
                self.precision=pd.read_csv(f'data/{self.version}_{self.dimension}d_{algorithm_portfolio}_column_normalized_score.csv',index_col=self.id_columns)
    ...
    def get_problems_to_use(self,budget):
        ...
        problems= list(set(self.ela.index).intersection(set(y.index)).intersection(set(list(self.ela.index))))
        return problems
